...
\begin{parts}
    ...
    \begin{solution}
    Triangular numbers are of the form \(\flatfrac{n(n+1)}{2}\). Favourable outcomes are pairs of the form \((n,\flatfrac{(n+1)}{2})\) or \((\flatfrac{n}{2},n+1)\)
    For \(n=1\), we have \((1,1)\).
    For \(n=2\), we have \(\{(1,3),(3,1)\}\).
    For \(n=3\), we have \(\{(3,2),(2,3)\}\).
    For \(n=4\), we have \(\{(2,5),(5,2)\}\).
    For \(n=5\), we have \(\{(5,3),(3,5)\}\).

    P \(=\flatfrac{9}{36} = \flatfrac{1}{4}\).
\end{solution}
    \part an even number
    \begin{solution}
    ...
    \begin{solution}
        The only triangular number that is also a square is 1. \(\{(1,1)\}\) is the only favourable outcome.
